...
output "egress_route" {
  ...
  value = {
    ...
    p2s_client_address_prefixes = [for route in azurerm_route.p2s_to_gateway : route.address_prefix]
    p2s_client_next_hop_type    = length(azurerm_route.p2s_to_gateway) > 0 ? "VirtualNetworkGateway" : null
  }
}
