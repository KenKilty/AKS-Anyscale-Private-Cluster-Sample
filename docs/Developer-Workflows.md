...
The plan-time Terraform tests (`infra/terraform/tests/*.tftest.hcl`) do **not** run inside `deploy`; they need a clean environment and run as a standalone gate via `terraform -chdir=infra/terraform test` (also wired into the quality-gate task).
...
az aks bastion ...        # or: ./scripts/utility/p2s-vpn.sh / ssh over the Bastion tunnel
...
# 3. Run the job/service proofs from the jump host (jump-host execution mode is
#    already forced in the jump host's .env by Module 2):
