...
| Mode | Where commands run | Notes |
| --- | --- | --- |
| `workstation` | Operator workstation/client | Default mode. Uses Azure CLI auth and a Bastion-backed kubeconfig for private AKS access. Terraform runs here. |
| `jump-host` | In-VNet Linux jump host | Uses managed identity and private network reachability. Runs post-configuration, proof, custom-image, and operational commands. Terraform must not run here. |
...
| `e2e` | Runs a full lifecycle path. Optional flags include jump-host mode, custom image, and teardown. |
...
The `e2e --mode jump-host --custom-image --teardown` path runs the same broad
...
- Keep generated or local run evidence out of commits unless it is explicitly
  intended documentation, such as `docs/RESULTS.md`.
...
| GPU sizing | Adjust `TF_VAR_gpu_pool_configs` and account for workspace/service capacity. |
...
- [`RESULTS.md`](RESULTS.md) - validation evidence.
