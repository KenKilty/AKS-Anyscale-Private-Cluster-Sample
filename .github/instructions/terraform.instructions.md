...
down the full lab. Provider `azurerm ~> 4.x`, Terraform `>= 1.9.0`. Changes must
stay aligned with the Microsoft Learn Anyscale-on-Azure, AKS, and ACR docs.

## azurerm v4 gotchas (verified the hard way)
...
`.tf` or checked-in `.tfvars.json`. Real values live only in git-ignored `.env` and
the rendered, ignored `terraform.auto.tfvars.json`. No personal checkout paths.
...
```
Run `terraform -chdir=<stack> test` when identity/private-mode contracts change.
