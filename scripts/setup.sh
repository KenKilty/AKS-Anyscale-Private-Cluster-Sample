...
render_tfvars() {
  ...
  export TF_VAR_anyscale_platform_admin_role_assignments
  ...
  jq -n \
    ...
    --arg linux_jump_host_vm_size "${TF_VAR_linux_jump_host_vm_size}" \
    --arg linux_jump_host_admin_username "${TF_VAR_linux_jump_host_admin_username}" \
    --arg linux_jump_host_admin_ssh_public_key "${TF_VAR_linux_jump_host_admin_ssh_public_key}" \
    --arg linux_jump_host_custom_data "${TF_VAR_linux_jump_host_custom_data}" \
    ...
    '{
      ...
      linux_jump_host_vm_size: $linux_jump_host_vm_size,
      linux_jump_host_admin_username: $linux_jump_host_admin_username,
      linux_jump_host_admin_ssh_public_key: $linux_jump_host_admin_ssh_public_key,
      linux_jump_host_custom_data: (if $linux_jump_host_custom_data == "" then null else $linux_jump_host_custom_data end),
...
require_cluster_kubectl_access() {
  ...
  if [[ "${ANYSCALE_EXECUTION_MODE:-workstation}" == "jump-host" ]]; then
    ensure_jump_host_cluster_access
    return 0
  fi
  use_bastion_kubeconfig_if_present
...
ensure_jump_host_cluster_access() {
  ...
  # Derive names from env (TF_VAR_*) rather than terraform output: the jump host
  # has no local Terraform state, and these match Terraform's resource_group_name
  # / aks_cluster_name outputs exactly.
  ...
  export_kubeconfig_env "${kubeconfig_file}"
  log "Using direct private AKS kubeconfig ${KUBECONFIG} (jump-host mode)"
  kubectl --request-timeout="${SETUP_TIMEOUT_KUBECTL_READY_SECONDS}s" get nodes -o wide
}

ensure_cluster_access() {
  # Execution-mode-aware private AKS access. Workstation mode keeps the Bastion
  # tunnel; jump-host mode uses direct private API access from inside the VNet.
  case "${ANYSCALE_EXECUTION_MODE:-workstation}" in
    jump-host)
      ensure_jump_host_cluster_access
      ;;
    *)
...
submitter_storage_private_dns_ready() {
  ...
  local storage_account host resolved_ip
  storage_account="$(operator_storage_account_name)"
...
# Guard the local (workstation) anyscale submit path. When ANYSCALE_CLI_TOKEN is
# unset the harness submits from the workstation, which must reach the PRIVATE
# storage account to upload the working dir. Fail fast with actionable guidance
...
# fast here with mode-aware guidance instead of part way through the pipeline.
