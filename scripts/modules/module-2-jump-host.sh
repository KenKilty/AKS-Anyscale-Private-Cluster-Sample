...
on_jump_host() {
  [[ "${ANYSCALE_EXECUTION_MODE:-workstation}" == "jump-host" ]]
}
...
module_2_bootstrap() {
  ...
  warn "ANYSCALE_EXECUTION_MODE is not 'jump-host'."
...
# ---------------------------------------------------------------------------
# doctor — execution-mode-aware readiness, delegated to the dispatcher.
# ---------------------------------------------------------------------------
module_2_doctor() {
  load_env
  "${DISPATCHER}" doctor "$@"
...
module_2_verify() {
  ...
  if [[ -f "${ROOT_DIR}/.env" ]] && grep -q '^ANYSCALE_EXECUTION_MODE=jump-host' "${ROOT_DIR}/.env" 2>/dev/null; then
    log "PASS: .env selects jump-host execution mode"
  else
    warn "FAIL: .env missing or not set to ANYSCALE_EXECUTION_MODE=jump-host"
    failures=$((failures + 1))
  fi
